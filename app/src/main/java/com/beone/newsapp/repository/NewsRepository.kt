...
class NewsRepository(val database: NewsDatabase) {
    ...
    val topNews: LiveData<List<TopNews>> =
        Transformations.map(database.topNewsDao.getNews(Category.General.title)) {
            it.asDomainModel()
        }
    ...
    suspend fun refreshNews() {
        try {
            withContext(Dispatchers.IO) {
                val newsList = service.getTopNews("us", Category.General.title)
                database.topNewsDao.insertTopNews(newsList.asTopNewsDatabaseModel(Category.General.title))
            }
